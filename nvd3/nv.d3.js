...
nv.models.scatter = function() {
  ...
  function chart(selection) {
    ...
    if(addDomainBuffer == true) {
        if(x.domain()[0] >= 0 && (x.domain()[0] - (xDomainDiff*bufferPerct) < 0))
            x.domain([0, x.domain()[1] + xDomainDiff * bufferPerct])
        else
            x.domain([x.domain()[0] - xDomainDiff * bufferPerct, x.domain()[1] + xDomainDiff * bufferPerct])
        y.domain([y.domain()[0] - y.domain()[0] * bufferPerct, y.domain()[1] + y.domain()[1] * bufferPerct])
    }
      // If scale's domain don't have a range, slightly adjust to make one... so a chart can show a single data point
      if (x.domain()[0] === x.domain()[1] || y.domain()[0] === y.domain()[1]) singlePoint = true;
      if (x.domain()[0] === x.domain()[1])
      ...
      //Info: If x/y is non numeric,display those bubbles at left-bottom corner
